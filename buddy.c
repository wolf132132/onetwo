...
int kmemfree(void)
{
    ...
    /* Get ord struct for MAX_ORD (12, 4KB blocks) */
    ord = &kmem.orders[MAX_ORD - MIN_ORD];
    next = ord->head;

    while(next != NIL) {
        /* Get mark struct from ord and blkid */
        mk  = get_mark(MAX_ORD, next);

        /* Count free blocks in mark using the bitmap */
        for (i = 0; i < 32; i++) {
            if ((mk->bitmap >> i & 0b1)) {
                count += 1;
            }
