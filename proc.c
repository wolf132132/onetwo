...
int generate_table(struct pinfo *proctable)
{
    struct proc *p;
    ...
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        if(p->state != UNUSED) {
	    proctable->pd[proctable->pcount].pid = p->pid;
	    proctable->pd[proctable->pcount].mem = p->sz;
	    strncpy(proctable->pd[proctable->pcount].name, p->name, 16);
	    proctable->pcount++;
       	}
